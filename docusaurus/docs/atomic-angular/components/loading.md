...
:::warning Deprecated
The `Loading` component and its `/loading` route are no longer used and are **deprecated**. Authentication is now handled at bootstrap (`withBootstrapApp` / `signIn`), and the `AuthGuard` redirects unauthenticated users straight to the login page. This component will be removed in a future major release — do not use it in new code.
:::
