...
Updates the application state with the provided `CCApp` object.
  ...
  ```ts
  update(app: Partial<CCAppState>): void
  ```
  
  | parameter | type                  | description                                             |
  |-----------|-----------------------|---------------------------------------------------------|
  | `app`     | `Partial<CCAppState>` | The partial application state to merge into the store.  |
...
Retrieves the count of items in the specified aggregation. Defaults to `10` if not defined.
...
| parameter         | type                  | description                                                                                 |
|-------------------|-----------------------|---------------------------------------------------------------------------------------------|
| `queryName`       | `string \| undefined` | The name of the query to retrieve the aggregation count for. Returns `10` when `undefined`. |
...
| `agents`           | Returns the `agents` customization object.                     |
...
Retrieves the `enableFieldedSearch` flag for a specific query.

Returns the `enableFieldedSearch` value for the specified query, or `false` if not found.
...
| parameter    | type     | description                                                                  |
|--------------|----------|------------------------------------------------------------------------------|
| `queryName`  | `string` | The name of the query for which to retrieve the enable fielded search flag.  |
...
export class SomeComponent {
  ...
  enableFieldedSearch: boolean = inject(AppStore).enableFieldedSearch('_query');
  // returns true or false depending the flag set in Sinequa Administration
}
```
...
| parameter   | type     | description                                                                               |
|-------------|----------|-------------------------------------------------------------------------------------------|
| `aggColumn` | `string` | The column name (or alias) for which to retrieve the customization.                       |
| `aggName`   | `string` | _(optional)_ The aggregation name — takes priority over column when both are provided.    |
...
Determines whether a specific agent is allowed based on its presence and configuration in the store.
...
| Parameter   | Type     | Description                      |
|-------------|----------|----------------------------------|
| `agentName` | `string` | The name of the agent to check.  |

```ts title="check-agent-allowed.ts"
const isAllowed = inject(AppStore).isAgentAllowed("myAgent");
// Output: true/false based on whether the agent exists and has defaultAgent configured
```
