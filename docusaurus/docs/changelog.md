...
- **Extracts and entities are painted on OCR'd documents again** (ES-28511). The PdfToHtml family shows the scanned image of the page and puts the OCR text under it, every word carrying an inline `visibility: hidden` — and the highlight spans are nested *inside* those words, so they inherited the invisibility. A highlight could therefore be scrolled to and still show nothing: neither its category colour nor the selection frame was ever painted. The generated highlight rules now declare `visibility: visible`, which overrides what is inherited. It is scoped to the categories currently switched on: unchecking one drops its rule and the word goes back to hidden, so no part of the text layer is revealed that the reader did not ask for. Measured on a real `_PdfAdvanced` capture — a `geo` box of 131.5 × 17 px whose computed visibility was `hidden` — no `!important` is needed, since the `hidden` sits inline on the parent word rather than on the highlight. Having the *entities* appear from expanding the advanced-search panel additionally needs `@sinequa/atomic-angular` ≥ 1.12.0 (sba-atomic-angular#562); until that release, the entities toggle button is what turns them on.
