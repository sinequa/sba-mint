...
- **Links inside the document are now inert** (`a { pointer-events: none }`). A preview is a viewer, not a navigable document: following a link out of a page sheet is not what the reader asked for, and on an OCR’d conversion the links the PdfToHtml family generates around any e-mail address or URL it finds are invisible yet present. If your integration relies on clickable links in the preview — a converted web page, typically — this is the rule to override.
