...
We attach the function `appInitializerFn()` which will try to recognize the authentication mode defined in the Administration.
...
*   If at least one mode is returned, it will be automatically used by the `login()` function to authenticate.
*   If no mode is returned, the application will attempt SSO authentication or default to login/password if SSO authentication fails.

:::note
You don't need to change anything in the application, the `autoLogin()` function takes care of everything.
:::
