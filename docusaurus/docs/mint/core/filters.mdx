...
*   or simply by creating your own component.
    👉 `core/components/filters/filters-list.component.ts`
...
So, if you want to simply add new filters, you just need to add a new entry in the list of `providers` in the `app.config.ts` file:
...
The number of visible items by default is 5, but it can be customized using the Token `FILTERS_BREAKPOINT` in the file 👉 `app.config.ts`.
```ts title="app.config.ts"
export const appConfig: ApplicationConfig = {
  providers: [
...
*   date
*   generic (applies to all filters that are not dates)
*   "more filters"
All these buttons are located in the folder 👉 `filters/buttons`
...
Each row is represented by the component `aggregation-row.component.ts`
...
|------------|--------------------------------|-----------------------------------------------------------------------|
| `onSelect` | `(event: AggregationListItem)` | Triggered when the row is selected.                                   |
| `onOpen`   | `(event: AggregationListItem)` | Triggered when a node is opened. Only used by tree-type aggregations. |
...
| `displayEmptyDistributionIntervals` | boolean                        | Indicates whether to display options with no results.<br/> Default is `false`.                                 |
