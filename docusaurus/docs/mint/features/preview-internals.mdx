...
Because the overlay lives inside the scaled body, the frames follow scroll and zoom for free. They are still recomputed on zoom, resize, scroll, description toggling and via a `ResizeObserver`, because changing `--factor` also changes the body's *layout* width (`width: calc(99% / var(--factor))`) and therefore reflows the content.
