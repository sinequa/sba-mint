...
2. Each rect is converted to the preview body's **local** coordinate space with `(rect - bodyRect) / factor`. This matters because `preview.css` puts `transform: scale(var(--factor))` on the body: the transform makes the body the containing block of its absolutely positioned descendants, so the overlay is positioned in pre-scale coordinates while `getBoundingClientRect()` reports post-scale viewport coordinates. The single division handles both the scroll offset and the scale.
...
4. One `.sq-passage-box` per block is appended to `#sq-passage-layer`, a zero-sized anchor pinned to the body origin.
