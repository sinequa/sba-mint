...
3. Rects are grouped into blocks: never across two page sheets (the body-level ancestor is the page), then by line (vertical overlap, small horizontal gap — the column gutter is what separates two columns) and by consecutive lines of the same column.
