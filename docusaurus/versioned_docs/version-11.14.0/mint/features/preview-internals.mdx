...
Because the overlay lives inside the scaled body, the frames follow scroll and zoom for free. They are still recomputed on zoom, resize, scroll, description toggling, on any `load` event inside the content document and once `document.fonts` is ready, because changing `--factor` also changes the body's *layout* width (`width: calc(99% / var(--factor))`) and therefore reflows the content.
