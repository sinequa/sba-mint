      ...
      <div class="col-start-2 col-span-2">
        ...
        <ul class="flex flex-col">
          @for (scope of history(); track $index) {
            <li
              role="presentation"
              class="my-3 text-xl font-semibold capitalize"
            >
            ...
            @for (search of scope.searches; track $index) {
              <li
                class="grid grid-cols-[auto_20%_min-content] p-1 group rounded-md hover:bg-blue-50 hover:cursor-pointer"
                role="link"
                ...
                <span class="text-gray-500">
                  ...
                  @if (search.filterCount) {
                    ...
                    <span class="font-semibold lowercase">
                      {{ search.filterCount }} {{ 'filters' | transloco }}
                    </span>
                  }
                </span>

                <button class="invisible group-hover:visible text-red-500" (click)="remove($event, search)">
...
export class RecentSearchesComponent {
  ...
  constructor() {
    effect(() => {
      ...
      untracked(() => {
        const groupedByDay = recentSearches.reduce((acc, search) => {
          ...
          if (search.queryParams?.filters)
            search.queryParams.filters = wrapFiltersToArray(search.queryParams.filters);
