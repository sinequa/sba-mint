  ...
  template: `
    <dialog popover class="z-backdrop w-full max-w-md rounded-lg border border-neutral-200 p-4 shadow-2xl" #dialog>
      <div class="flex flex-col gap-4">
        <h1 class="text-xl font-bold">{{ 'alert.createAlert' | transloco }}</h1>
        ...
        <div class="mt-4 flex justify-end gap-2">
          @if (alert) {
            <button class="btn btn-tertiary w-24 outline-none" (click)="execute()">
              {{ 'alert.execute' | transloco }}
            </button>
          }
          @if (canUpdateQuery()) {
            <button class="btn btn-tertiary w-24 outline-none" (click)="updateQuery()">
              {{ 'alert.updateQuery' | transloco }}
            </button>
          }
          <button class="btn btn-tertiary w-24 outline-none" (click)="confirm()" [disabled]="invalidForm()">
            {{ 'confirm' | transloco }}
          </button>
